/*
  ...
  ---------------------------------------------------------------------
  Sept 11, 2017			Steve Young			Initial Version
...
DECLARE @TableName nvarchar(50) --Variable to house the current table name
...
--where table_name like 'Clean%'
--where table_name = 'Clean_LeapKnee01'
order by Table_name   -- Order by name
...
=================================================================== */
Declare @i int
select @i = min(RowID) from @userTables
declare @max int
select @max = max(RowID) from @userTables

--Loop Begin
while @i <= @max begin

--Grab the current table
Set @TableName = (select Tablename from @userTables where RowID = @i) --do some stuff
...
PRINT '<tr>'
SELECT 
    '<td>' + isnull(ltrim(rtrim(ORDINAL_POSITION)),'NULL') + '</td>',
    '<td>' + isnull(rtrim(ltrim(COLUMN_NAME)),'NULL') + '</td>',
	  '<td>' + isnull(rtrim(ltrim(DATA_TYPE)),'NULL') + '</td>',
	  '<td>' + isnull(rtrim(ltrim(CHARACTER_MAXIMUM_LENGTH)),'NULL') + '</td>',
	  '<td>' + isnull(rtrim(ltrim(IS_NULLABLE)),'NULL') + '</td>',
	  '<td>' + isnull(rtrim(ltrim(COLUMN_DEFAULT)),'NULL') + '</td>',
	  '<td></td></tr>'
FROM   
